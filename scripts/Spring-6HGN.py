...
def main(N=5,epochs=10000,seed=42,rname=True,saveat=10,error_fn="L2error",dt=1.0e-3,ifdrag=0,stride=100,
            trainm=1,grid=False,mpass=1,lr=0.001,withdata=None,datapoints=None,batch_size=100,config=None,noise_per=0):
    print("Configs: ")
    ...
    absmean = jnp.abs(Zs_dot).mean()
    datastd = 3.850146258460286 #same as LGN and HGN
    noise = np.random.normal(0,(0.01*noise_per)*datastd,Zs_dot.shape)
    Zs_dot = Zs_dot+noise
    ...
    if datapoints is not None:
        allZs = allZs[:datapoints]
        allZs_dot = allZs_dot[:datapoints]
    
    Ntr = int(0.75*len(allZs))
    Nts = len(allZs) - Ntr
    ...
    stop = time.time()
    savefile(f"training-time-{np.round((stop - start),2)}-s", [])
