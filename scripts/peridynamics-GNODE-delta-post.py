...
# jnp.hsplit(apply_fn(R, V, params["pos_vel_params"]),2)

def x_v_fn_model(x, v, params): return jnp.hsplit(apply_fn(x, v, params["pos_vel_params"]),2)

params = loadfile(f"perignode_trained_model.dil")[0]

Rs_pr,Vs_pr = x_v_fn_model(Rs[0], Vs[0], params)

def get_forward_sim(params=None, force_fn=None, runs=10):
    @jit
    def fn(R, V):
        return predition_delta(R,  V, params, force_fn, shift, dt, masses, stride=stride, runs=runs)
    return fn


sim_model = get_forward_sim(params=params, force_fn=x_v_fn_model, runs=runs)
...
for ind in range(maxtraj):
    ...
    nexp["z_actual"] += [origin_Rs[1:1+runs]]
    nexp["Zerr"] += [RelErr(origin_Rs[1:1+runs], pred_traj.position)]
    # nexp["AbsZerr"] += [AbsErr(origin_Rs[1:1+runs], pred_traj.position)]
    nexp["AbsZerr"] += [jnp.abs(norm(origin_Rs[1:1+runs]) - norm(pred_traj.position))]
    
    ac_mom = jnp.square(origin_Vs[1:1+runs].sum(1)).sum(1)
    pr_mom = jnp.square(pred_traj.velocity.sum(1)).sum(1)
    # nexp["Perr"] += ([RelErr(origin_Vs[1:1+runs], pred_traj.velocity)])
    nexp["Perr"] += ([RelErr(origin_Vs[1:1+runs]
                     [6:], pred_traj.velocity[6:])])
