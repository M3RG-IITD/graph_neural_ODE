...
def main(N=3, dt=1.0e-3, useN=None, withdata=None, datapoints=100, mpass=1, grid=False, stride=100, ifdrag=0, seed=42, rname=0, saveovito=0, trainm=0, runs=200, semilog=1, maxtraj=100, plotthings=False, redo=0):
        ...
        try:
            ...
            if plotthings:
                if ind<5:
                    ...
                    plt.savefig(_filename(f"FGN {N}-Spring Exp {ind}".replace(" ", "-")+f"_actualH.png"))               
                else:
                    pass
            # Es = Es_fn(actual_traj)
            # H = Es[:, -1]
