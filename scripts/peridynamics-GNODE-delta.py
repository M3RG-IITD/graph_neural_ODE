...
ff2_params = mlp(Nei, dim+dim, key)
...
# x, p = jnp.hsplit(apply_fn(R, V, Fparams), 2)
...
v_x_v_fn_model = vmap(x_v_fn_model, in_axes=(0, 0, None))

# __, _ = v_x_v_fn_model(Rs[:10], Vs[:10], params)
...
def loss_fn(params, Rs, Vs, Fs):
    Rs_pr,Vs_pr = v_x_v_fn_model(Rs, Vs, params)
    return MSE(Rs_pr[:-1], Rs[1:])+MSE(Vs_pr[:-1], Vs[1:])

# loss_fn(params, Rs[:10], Vs[:10], Fs[:10])
