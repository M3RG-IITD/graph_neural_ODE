...
params = loadfile(f"perimcgnode_trained_model.dil")[0]
...
for ind in range(maxtraj):
    ...
    nexp["Zerr"] += [RelErr(origin_Rs[runs*ind:runs +
                            runs*ind], pred_traj.position)]
    # nexp["AbsZerr"] += [AbsErr(origin_Rs[runs*ind:runs+runs*ind], pred_traj.position)]
    nexp["AbsZerr"] += [jnp.abs(norm(origin_Rs[runs *
                                ind:runs+runs*ind]) - norm(pred_traj.position))]
    ...
    nexp["Perr"] += ([RelErr(origin_Vs[runs*ind:runs+runs*ind]
...
# make_plots(nexp, "AbsZerr", yl=r"${||\hat{p}-p||_2}$")
